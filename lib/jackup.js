...
parser.option("--wildfire", "wildfire")
    .help("enable wildfire support")
    .bool();
...
exports.main = function main(args) {
    ...
    if (options.wildfire)
    {
        // only enable wildfire if installed as a using package
        var wildfireUid = "github.com/cadorn/wildfire/zipball/master/packages/lib-js-system";
        if (wildfireInfo = require("packages").usingCatalog[wildfireUid])
        {
            // add wildfire system package to require.paths
            require.paths.push(wildfireInfo.directory.join("lib"));
        }
    }
    ...
    if (options.wildfire && wildfireInfo)
    {
